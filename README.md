...
Класс IPaymentForm отрисовывает окно с формой содержащей способ оплаты и адрес пользователя, является дженериком, расширяет класс Form и принимает в переменной интерфейс IOrder.

Конструктор класса:
    constructor(container: HTMLFormElement, events: IEvents) - принимает элемент контейнера и объект событий.

Методы класса IPaymentForm:
...
Класс IContactForm отрисовывает окно с данными пользователя (телефон и e-mail), является дженериком, расширяет класс Form и принимает в переменной интерфейс IOrder.

Конструктор класса:
    constructor(container: HTMLFormElement, events: IEvents) - принимает элемент контейнера и объект событий.

Методы класса IContactForm:
