...
В конструктор класса Component<T> передается элемент контейнера, в который будет помещен компонент.
...
- setVisible - отображает переданный элемент4
...
В конструктор класса приходят: сonstructor(data: Partial<T>, protected events: IEvents)
