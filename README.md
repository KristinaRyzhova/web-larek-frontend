...
В конструктор класса ```text Component<T>``` передается элемент контейнера типа HTMLElement, в который будет помещен компонент.

Методы класса ```text Component<T>```:
...
- data: ```text Partial<T>``` - данные, принимают любое количество свойств типа T;
